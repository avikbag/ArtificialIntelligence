...
Puzzle aStarHeu(Puzzle root) // pass in root node to search for a star path using manhattan as heuristic
{
  visited.clear();
  path.clear();
  unexpanded.clear();
  ...
  vector<char> moves;
  visited.push_back(make_tuple(root, root.heuristicVal()));
  Puzzle temp = root;
  Puzzle tempSt;
  int min = 100000;
    ...
    for(int i = 0; i< elements.size(); i++)
    {
      ...
      for(int j = 0; j < moves.size(); j++)
      {
        ...
        if (!check_exists(visited, tempSt))
        {
          ...
          visited.push_back(make_tuple(tempSt, tempSt.heuristicVal()));
          unexpanded.push_back(make_tuple(tempSt, tempSt.heuristicVal()));
        }  
      }
    }
